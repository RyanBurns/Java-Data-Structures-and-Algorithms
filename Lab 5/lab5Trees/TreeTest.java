...
public class TreeTest {
	private Stack<Node> stack = new Stack<Node>(); //Our custom stack for stackDFS()
	
	public TreeTest() {
		Node root = new Node(); //Root is the base of the tree
	...
	public void stackDFS(Node root){ //This DFS uses a custom stack
		stack.push(root); //Push root of tree to the top of the stack
		while (!stack.isEmpty()){ //Keep processing until the stack is empty
			Node node = stack.pop(); //Get and remove from the top of the stack
			System.out.println(node.getItem().process()); //Process the node
	...
	private void init(Node root){
		...
		Node robert = new Node(new Person("Robert")); //Duke of Normandy, eldest son of William I
		...
		Node adela = new Node(new Person("Adela")); //Daughter of William I.
		adela.addChild(new Node(new Person("Stephen"))); //King of England. Reign was marked by civil war with cousin Matilda.
		root.addChild(adela);
		
		Node henryI = new Node(root, new Person("Henry I")); //King of England
		root.addChild(henryI);
		henryI.addChild(new Node(new Person("William"))); //Dead at 17
		Node matilda = new Node(new Person("Matilda")); //Matilda of England/Maude. Tough lady. Civil war against Stephen. 
		henryI.addChild(matilda);
		
		Node henryII = new Node(root, new Person("Henry II")); //Henry Plantagenet. This is the guy who launched the Norman invasion of Ireland in 1169
